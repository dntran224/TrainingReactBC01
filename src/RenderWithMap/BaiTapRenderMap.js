...
export default class BaiTapRenderMap extends Component {
  dataFilm = [
      ...
      },
      {
        ...
        hinhAnh:
          "https://movie0706.cybersoft.edu.vn/hinhanh/thelongestride.jpg",
        moTa:
      ...
      },
      {
        ...
        hinhAnh:
          "https://movie0706.cybersoft.edu.vn/hinhanh/thewalkingdead.jpg",
        moTa:
  ...
  // Viết hàm hiển thị danh sách phim dưới dạng div card boostrap
  renderFilm = () => {
    const danhSachFilm = this.dataFilm.map((dsFilm, index) => {
      return (
        <div className="col-4" key={index}>
          <div className="card text-left">
            <img className="card-img-top" width="150" height="450" src={dsFilm.hinhAnh} alt={dsFilm.hinhAnh}/>
            <div className="card-body">
              <h4 style={{height:50}} className="card-title">{dsFilm.tenPhim.length >50 ? <p>{dsFilm.tenPhim.sub(0,100)} ...</p> : <p>{dsFilm.tenPhim}</p>}</h4>
              <div style={{height:80}} className="card-text">{dsFilm.moTa.length > 100 ? <p>{dsFilm.moTa.substr(0,100)} ...</p> : <p>{dsFilm.moTa}</p>}</div>
            </div>
          </div>
        </div>
      );
